...
install() {
    ...
    case "$target" in
        st|dmenu|dwm|dwmbar|xmouseless|htop-vim|sshrc|ctags|xwallpaper|acpilight|brillo|ncmpcpp|neovim)
            sudo make install
            ;;
...
progs=(st dmenu dwm dwmbar dotfiles df dragon xmouseless term-theme brillo htop-vim
       sshrc fzf ctags zsh-as zsh-fsh xwallpaper acpilight libxft-bgra ncmpcpp neovim lf xurls)
